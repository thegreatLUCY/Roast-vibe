...
AI codegen tools (Lovable, Bolt.new, v0, Replit Agent, Cursor, Codex, and other coding-agent workflows) ship apps that *look* finished — but leak credentials, fake auth in the browser, ignore Row Level Security, store JWTs in `localStorage`, and accumulate three state-management libraries that do the work of zero.
...
> 🟢 *"checks the internet, passes the audit"* — a clean utility repo (95/100)
>
...
Rules across 5 buckets. Same repo + same commit → same score, forever. The LLM never touches the number.

### 🤖 Tool fingerprinting
Detects **Lovable**, **Bolt.new**, **v0**, **Replit Agent**, **Cursor**, **Codex**, and other coding-agent fingerprints to flavor the roast without affecting the score.
...
### 💸 Zero infra cost
Runs entirely on Cloudflare's free tier. The only paid line is OpenRouter, with model choice controlled by environment config.
...
[ 1. Fetch + filter ]      GitHub API. Skip node_modules/dist/lockfiles.
      ...
      ▼
[ 2. Scan ]                Deterministic rules across 5 buckets.
...
|---|---|---|
| 🚨 **Secrets** | -50 | Committed `.env`, LLM provider keys, Stripe / AWS / GitHub keys, Supabase `service_role` JWTs, hardcoded secret fallbacks |
| 🔐 **Auth & DB** | -30 | API routes with no auth check, SQL string concat, Supabase without RLS migrations, CORS wildcards, `localStorage` token storage, client-only auth |
| 🤡 **AI slop** | -20 | `// In a real app you would...` comments, `mockUsers` shipped to prod, hardcoded production data, browser-only business-data persistence, fake frontend checkout |
| 🏷️ **Tool classifier** | 0 *(flavor)* | Lovable / Bolt.new / v0 / Replit Agent / Cursor / Codex / agent-workflow fingerprinting |
| ⚠️ **Quality smells** | -5 | Multiple state libraries, async with no try/catch, mega-components |

Final score starts with capped deductions, then applies confidence weighting, corroboration bonuses, severe-risk ceilings, and floors so cosmetic/vibe-only findings do not masquerade as production catastrophes. Real-looking committed `.env` files cap at 50, `.env` plus missing Supabase migrations caps at 45, repeated unauthenticated data APIs cap at 40, and real server/API CORS wildcard caps at 89.
...
- 💾 **Result caching:** keyed by `owner/repo@commit-sha`, identical resubmissions are free
- 📏 **Prompt-size cap:** LLM input is limited to the top findings and snippets
...
✅ **Phase 1** — Scan pipeline backend, scoring, and LLM roast
...
✅ **Phase 4** — Calibration with 40+ public AI-generated repos
🔄 **Phase 5** — Production deploy verification + refreshed screenshots
...
Built solo. Powered by Cloudflare Workers + OpenRouter.
