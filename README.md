...
AI codegen tools (Lovable, Bolt.new, v0, Replit Agent, Cursor, Claude Code, Codex) ship apps that *look* finished — but leak credentials, fake auth in the browser, ignore Row Level Security, store JWTs in `localStorage`, and accumulate three state-management libraries that do the work of zero.
...
### 🤖 Tool fingerprinting
Detects **Lovable**, **Bolt.new**, **v0**, **Replit Agent**, **Cursor**, **Claude Code**, and **Codex** apps and flavors the roast accordingly.
...
|---|---|---|
| 🚨 **Secrets** | -50 | Committed `.env`, OpenAI / Anthropic / Stripe / AWS / GitHub keys, Supabase `service_role` JWTs, hardcoded secret fallbacks |
...
| 🏷️ **Tool classifier** | 0 *(flavor)* | Lovable / Bolt.new / v0 / Replit Agent / Cursor / Claude Code / Codex fingerprinting |
